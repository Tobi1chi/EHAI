...
_PLANNER_SYSTEM_PROMPT = "\n".join(
    (
        ...
        "phase or branch boundary, use set_node_gate on an existing work or merge node; do not",
        "add a Gate for every small task, and never attach one to a fork, evaluator, or final",
        "node.",
        "A node Gate is an automatic command check only. Do not encode human judgment, a Stage",
        "Reviewer decision, or a manual Gate as a command; those interfaces are not available",
        "in this increment.",
