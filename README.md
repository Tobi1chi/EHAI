...
P1 历史原型完成；P2 已有可用开发核心，Git 整合与后继 Run 已接通，总验收和长任务恢复仍需收尾。
...
过程查询提供 block 版本/变化清单；integrate-run 自动整合当前有效代码成果。
重新批准后的 start-run 可显式接续旧 Run 的完成成果，并重新执行新 Gate。
未决事项处置、文件格式和适用条件见 [Usage](docs/USAGE.md)。
