...
| R2 获批方案到代码 | 指定 workspace 执行获批任务，调度 Worker，整合并交付选中代码 | 代表性 Built-in 真实 CLI 编码通过：三槽并行、双代码分支、选中成果整合、单一最终 Gate；完整关闭恢复、长运行和 Server 实跑仍待验证 | 先 Built-in 再 Codex App Server；不复制 Agent Loop 或静默扩大批准范围 |
