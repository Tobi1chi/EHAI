...
2026-09-14 优先级更新：用户决定采用完整 Pi 作为外部 Agent 后端，EHAI 不再维护模型 API、
工具循环和上下文算法。当前先按 [ADR 0005](adr/0005-external-agent-backends.md) 的迁移顺序
推进；随后用户明确允许直接删除旧 Runtime，现已移除并把各角色接到 Pi。
下表及旧自举记录保留背景，不再驱动继续扩建 Built-in Runtime；真实 Pi 验收尚未完成。
当前接入和验证范围见 [Pi 后端迁移记录](PI_BACKEND_MIGRATION.md)。
