...
| R2 获批方案到代码 | 指定 workspace 执行获批任务，调度 Worker，整合并交付选中代码 | 真实 CLI 已验证并行/分支编码；补充固定 Gate 失败修复和 Ctrl+C 后同一 Run 立即恢复，已完成上游不重跑；任意关窗/强杀、写入途中恢复、长运行和 Server 实跑仍待验证 | 先 Built-in 再 Codex App Server；不复制 Agent Loop 或静默扩大批准范围 |
