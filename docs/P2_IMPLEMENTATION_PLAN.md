...
| R2 获批方案到代码 | 指定 workspace 执行获批任务，调度 Worker，整合并交付选中代码 | 已接入详细任务、最终 Gate、代码快照/衔接、前台执行和续接入口；真实 Worker 试用等待第三方数据发送授权，不标记验收完成 | 先 Built-in 再 Codex App Server；不复制 Agent Loop 或静默扩大批准范围 |
