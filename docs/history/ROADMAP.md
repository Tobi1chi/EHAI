...
2026-09-14 当前实施路线改为 [外置 Agent 后端](../adr/0005-external-agent-backends.md)：
使用完整 Pi 接入规划、执行和审查，退役自研模型循环；保留 EHAI 上层任务语义与唯一产品验收。
用户随后决定直接删除旧底层，已移除自研 Runtime 并接线各角色；下一步验证真实正常执行闭环，
不把历史自举的长尾修复继续作为新底座，也不以接线代替验收。
