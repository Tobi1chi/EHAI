# 历史快照

这些文件是 2026-09-15 文档收口前的快照，只用于追溯设计、失败和试跑证据。
其中“当前”“下一步”、旧终端/Run 状态、模型偏好和命令参数都属于当时上下文；
不表示今天仍可用，不作为恢复退役 Runtime 或旧测试套件的要求。

当前入口从 [Usage](../USAGE.md) 开始，能力看 [STATUS](../STATUS.md)。

- [原 README](README_PRE_CONSOLIDATION.md)
- [产品基线](PRODUCT_SCOPE.md) · [执行模型](EXECUTION_MODEL.md) · [路线图](ROADMAP.md)
- [P1](P1_IMPLEMENTATION_PLAN.md) · [P2](P2_IMPLEMENTATION_PLAN.md) · [R2 详细记录](R2_IMPLEMENTATION_PLAN.md)
- [Pi 迁移](PI_BACKEND_MIGRATION.md) · [迁移批次](MIGRATION_STATUS.md)
- [自举试跑](SELF_HOSTING_GOAL.md) · [执行契约](P2_EXECUTION_CONTRACT.md)
- [用法快照](USAGE.md) · [迁移前用法](USAGE_PRE_PI.md) · [旧开发规范](DEVELOPMENT_GUIDELINES.md)

指向退役源码的历史链接可能仅在对应 Git 版本存在。不要按历史段落直接执行旧命令。
