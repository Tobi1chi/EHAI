...
## 2026-09-15：CLI 封装既有 API 能力（R4 入口收口）

用户要求通过 CLI 使用框架。既有本地 CLI 已覆盖规划、批准、前台执行及查询；本轮补齐
连接正在运行的 API 宿主的能力，不另建工作流状态机，不把尚未完成的后继 Run 接手包装为可用命令。

- 用户结果：人或上层 Agent 用 `ehai --api-url` 操作同一 API 宿主，不再必须手写 HTTP 脚本；
  客户端不打开 SQLite、不重复装配 Scheduler/Pi。宿主继续拥有运行、授权及进程收敛。
- 输入为现有命令参数及显式 API URL；客户端只组装已存在的路由/请求，输出 API `data` JSON。
  HTTP 错误保留状态和服务端正文到 stderr，退出 2；失败不重试、不跟随重定向、不回退本地。
  可选 socket 超时不改变任务期限，客户端 Ctrl+C 不暗中取消宿主工作。
- 复用 create/discuss/propose/approve/start、过程提案/审查/应用、Run 控制、人工回复和查询名称。
  新增 API-only 的健康/Worker/Attempt/待答请求查询，以及审查驱动 `suspend-attempt`、
  `cancel-attempt`、deadline 延长、Worker request resolve/decline。每个写入仍传业务幂等键；
  挂起仍必须绑定 review_id、through_sequence、actor/reason，不自动采纳审查意见。
- API `start-run` 要求完整 HTTP execution_config JSON 和 `--authorize`；实际批准及配置匹配
  仍由宿主检查。配置中的路径在宿主解析，客户端不访问模型凭证或宿主 Pi 目录。
  原 `execute-plan` / `resume-session` 前台生命周期保留；不能在 API 模式调用本地恢复/探查命令。
- API 模式不与本地存储/模型配置混用。本地模式继续要求 database/artifacts，纯 `inspect-agent`
  不再要求无用数据库路径。没有新增依赖、API/数据库 Schema，也未改变模型工具或角色提示词。

正常入口验证：在仓库外 `ehai-cli-api-20260915` 创建隔离数据库，启动真实 `uv run ehai-api`
回环宿主，使用 single 确定性 Planner / fake 后端，仅进行客户端与持久化验证，不调用模型。
新 CLI 经 HTTP 完成健康查询、项目/目标创建、提案、批准、方案查询和 Worker preset 查询；
方案 `7c8b99a6-51a9-4847-8154-29d61b331e79` 在本地及 API 模式返回相同 JSON。
同键同请求复用原 Project，同键不同请求返回 HTTP 409；未知方案返回 404；定向挂起错误
Attempt 到达宿主并返回 409/state_conflict。缺少执行授权在发请求前拒绝，未创建 Run。
只读核对数据库有 1 Project、1 Goal、1 PlanRevision、0 Run、0 Attempt；宿主已停止，
无残留试用进程，断开后的查询返回结构化 URLError，未回退数据库。没有新建常驻或临时测试套件。

静态核对所有 CLI HTTP 映射存在于当前 OpenAPI；Ruff check、format（140 文件）、mypy
（108 源文件）和 diff whitespace 检查通过。详细使用契约见 Usage 的 CLI API 模式章节。
本轮未验证 API 客户端启动真实 Pi 开发、活跃 Worker 正向挂起/请求处理/延长截止时间、HTTP
等待中 Ctrl+C 或异步过程调整完整运行；没有据此宣布模型能力或 P2 总验收完成。

## 2026-09-15：Planner 草稿 Gate 的定向修复入口

用户确认先补齐框架配套，Agent 使用指导 Skills 后置。本次只修复已观察到的草稿 Gate
编辑缺口，不调整角色 Prompt、不新增 Skills、不扩建 Runtime 或长期测试。

**故障与原因：** OpenCode Go 试用 `pi-go-ehai-capabilities-20260915` 中，Planner 在最终
Reviewer 上同时设置节点 Gate 和最终 Gate，`finish_plan` 返回 `NODE_GATE_FINAL_CONFLICT`。
随后以 `argv=[]`、`human_question=null` 试图清除节点 Gate，被正确拒绝为空定义；工具集却没有
直接删除 Gate 的入口，模型转向拆改阶段/节点，最终触达试用预算，未保存方案。
`remove_plan_node` 本来存在，但不是保持图结构的 Gate 修复操作；本次不将问题归因于缓存或网络。

**模块契约与实现：**

- 用户结果：Planner 可纠正错误 Gate 配置而不拆改任务图；输入是明确的草稿 Gate 目标，
  输出沿用 `accepted`、操作预算或结构化 `issues`，状态可用 `inspect_plan` 重读。
- `remove_node_gate(node_key)` 删除整个节点草稿 Gate；保留节点、边、Phase 和最终 Gate。
  已存在节点无 Gate 时幂等成功，未知节点拒绝；重复操作仍受原预算限制。
- `remove_final_gate({})` 清空草稿最终命令/人工条件，未设置时成功，不删除宿主完成条件。
  必需最终/阶段 Gate 仍由 `finish_plan` / `PlanTemplate` 校验，不能以删除绕过提交要求。
- 设置工具仍是完整替换，空数组清命令、null 清人工条件，两者均空不代表删除。
  Schema/描述与错误说明同步，冲突信息指出具体 `node_key` 和实际可用的修复工具。
- 所有权仍在 `PlanGraphToolRuntime` 的内存草稿；成功提交后由现有 Builder 生成 CheckSpec、
  CompletionContract 和 PlanRevision，正常应用服务保存。不直接删除数据库 Gate 或修改批准记录。
- `_planner_registry` 从 ToolSet 动态注册 Handler/角色工具名，Pi bridge 原样转交参数和结果；
  两个新工具使用关闭对象、全部字段显式必填的 Schema，不含 `uniqueItems`，未禁用 strict。
  无新增公开 HTTP 字段、CLI 命令或数据库迁移，无需重生成 Client。
- 过程模式不注册 Gate 设置/删除工具，Handler 仍拒绝直接调用；冻结的 Gate/Check 不可删除或
  重写，原 `move_process_gate` 只整体移动归属的能力保留。成功 finish 后不能再修改草稿。

**验证与范围：**

1. 仓库外 `ehai-gate-repair-20260915/test_gate_repair.py` 对真实故障做最小诊断：修复前在新
   删除入口失败为 `UNKNOWN_TOOL`；修复后两项通过，覆盖冲突清除、图/最终条件保留、空定义拒绝、
   删除幂等、必需最终 Gate、finish 后不可写，以及过程模式 Handler/注册保护。
   过程保护诊断只切换内存模式，不冒充已批准过程调整的真实模型验收。
2. 经生产 `create_local_app` 和正常 HTTP create/propose/get-plan/get-plan-checks，在隔离
   workspace 使用完整 Pi 0.85.1 + `deepseek-v4.1-flash`、thinking off，无 mock 或直接写领域状态。
   首轮 12 个请求均 HTTP 200，实际 `remove_final_gate` 和随后恢复成功；本地 12 请求上限在
   第一次冲突校验后阻止继续，故无持久方案。该停止是诊断预算，不是网关故障或 Gate 通过。
3. 保留首轮，精简为只验证剩余重复节点 Gate 的复测，共 8 个请求均 HTTP 200。模型实际收到
   `NODE_GATE_FINAL_CONFLICT` 后，调用 `remove_node_gate({"node_key":"review_hello"})`，
   紧接着 `finish_plan` 成功；之间没有节点/边/Phase 重建，也没有替换最终 Gate。
   此轮另有两次原有 `add_plan_node` 调用误把 JSON null 写成字符串 `"null"`，Pi 参数校验拒绝后
   模型改用 `session_policy="new"` 成功；没有放宽该约束，不将这两次拒绝隐去。
4. 正常查询及只读数据库核对：方案 `5cf8fcc6-a36e-46aa-9c26-1caf1c2ef727` 为未批准 draft，
   2 节点、1 依赖边、1 Phase；只有 1 项必需 command Check，归最终 Reviewer，argv 仍为
   `uv run --no-project python verify.py`，没有节点重复 Check 或人工条件；Run/Attempt 均为 0。
   两轮合计报告 152,932 token，其中输入缓存 132,608、未缓存输入 14,017、输出 6,307；
   这是上游 usage 总量，不是按单一费率计算的费用。
5. Ruff check、format check（139 文件）、mypy（107 源文件）及 diff whitespace 检查通过。
   原请求/响应、Pi 原生工具参数/结果及审计在系统临时目录 `ehai-gate-repair-20260915` 与
   `ehai-gate-repair-20260915-retry`，各有 `audit.json`；诊断、凭证和产物不进 Git。

本次证明工具定义接受、真实删除/修复行为及方案保存，不证明模型永不出错，也不包括批准、Worker
编码、运行 Gate、原先五节点并发任务或唯一产品 E2E。后续仍按正常产品能力收口，再共同确定总验收。
...
### 2026-09-14 显式审查决策到节点级挂起

用户确认实现：上层采纳轨迹意见后，只停止目标 Worker 当前承担的节点执行，保留路线和成果，
让其他独立工作继续，后续由现有回复/过程调整恢复。审查者仍仅给意见，不自动控制 Worker。

输入为 attempt_id、最新完成 review_id、调用方确认的 through_sequence、actor、reason 和业务幂等键。
TrajectorySuspensions 校验归属和最新完成意见、Run/Attempt/节点仍活跃、Session/过程/批准版本与
目标没有变化；持久化带指纹的 CommandReceipt 后交由 RuntimeControl/ConcurrentRuntime 停止。
不把模型 verdict 当作权限；显式调用者可以记录理由作不同决定。已保存的请求不盲目重送取消。

SingleSlotRuntime 先标记目标挂起意图，再取消该 Pi 执行并等待宿主工作；其终止失败/阻塞事件
进入已有 block_attempt，而不是原来会 fail node 并可能 pause 整 Run 的 interrupt_attempt 路径。
ConcurrentRuntime 只操作该 Attempt 的 helper/task，不修改整 Run 调度开关。
真实完成赢得完成/取消竞态，不伪造 blocked；响应同时给出操作状态与当前节点/Attempt 状态。
目标变为 interrupted + blocked、intervention 保存审查证据与调用方决定，后续沿既有恢复规则。
既有有效 handoff/工作区保留，不把脏状态自动升级为可恢复成果，也不声称外部效果已回滚。
控制决定跨 HTTP 请求持久化，但没有新增自动恢复未知取消、Planner 自动调用或权限系统。

正常入口为 POST /api/v1/attempts/{attempt_id}/suspend，生成 Client 的
suspendAttemptFromReview；请求与结果 Schema、OpenAPI 已同步。此接口要求目标活跃 API 宿主，
前台 CLI 不新增跨进程控制。过程调整仍走原提案、独立边界审查和应用，介入证据复用原上下文。

验证：仓库外 ehai-suspend-trial-20260914/run_trial.py 使用生产 API 装配及真实 lifespan，
通过 HTTP 创建/无模型 single 规划/批准/启动两个独立 Run；不直接写领域状态或 monkeypatch。
试用阈值为 2 步，Luna/high、capacity=2，唯一允许的命令是 65 秒等待后打印，无文件/远端副作用。
初始临时脚本使用了当前 FastAPI 不提供的 startup 方法，启动模型前改用原生 lifespan；不是产品故障。

- 目标 Run 3a66d01d-075a-48b5-a25f-96e091b93803，Attempt fa327356-955d-470f-901f-bc6263b598eb，
  Session 3245d45e-5cac-4faf-8421-06a4f4460b81；采纳 review fd1738a5-d363-4db9-abf7-059286bd1b75。
  试用操作者显式要求挂起以验证控制，与模型 continue 建议区分，不把它当作发现真实漂移的证据。
- HTTP 返回 suspended，Attempt interrupted、节点 blocked；intervention
  f9c06ef4-ead2-5f9f-99f0-66791a258ee4 包含原意见/序号、actor、reason、请求指纹与原图基线。
  相同请求重发返回同一介入；新幂等键对已结束 Attempt 使用旧意见返回 409，不触发新取消。
- 同宿主另一 Run 9f836286-51da-4cab-a576-97e31507d3f5 在目标挂起时仍 running，随后正常 completed。
  目标 Run 未被整体 pause，阻塞节点在另一任务完成期间没有被重派。两个 Attempt 工作区均保留。
- 宿主关闭后，经原 HTTP reply 接口核实本次无外部/文件副作用并保持批准边界，介入 replied、
  目标节点 pending。没有再启动 Worker 模型；这证明解除阻塞，不是完整恢复执行验收。
- Worker reported token 合计 23606，轨迹审查合计 4768，总计 28374，均为 Luna。
  控制请求本身不调用模型。临时仓库无源码修改，测试宿主正常结束，没有新增长期测试。
  Ruff/格式/mypy（107 源文件）、Schema 生成、Client generate/typecheck/build 通过。

边界：本次两个 Worker 属于不同 Run；同一图的多分支继续/下游等待仅完成代码路径检查，
还需正常任务实测。未覆盖实际写入中的取消、有效 handoff 续跑、图调整后模型执行、宿主崩溃和
完成/取消竞态，不宣称唯一产品 E2E 或全部节点级恢复行为已验收。

### 2026-09-14 周期轨迹审查：只保存意见，不自动干预

用户确认采用 10 分钟或 30 个执行步先到触发的折中方案，短任务不补审。
本次不继续用逐步审核或 Worker 自报来宣称确定性阻止漂移；保留前一轮失败证据。

实现所有权：TrajectoryReviewPolicy 随新 Run 的 execution_config 固定，缺省不启用，
显式空对象采用 600 秒/30 步/Luna 默认值。Worker Connector 管理旁路 watcher 的生命周期，
PiRoleRunner 记录有效模型响应及工具批次完成的 execution_step，排除 finish 后的空 abort。
每个 Attempt 一次只运行一个独立 Pi 审查 Session；不阻塞 Worker 模型/工具循环。
已完成 Worker 不补审，未完成审查随 Worker 结束取消并记账，错误不改变 Worker 执行结果。

审查输入是绑定当前节点/过程/批准边界的不可变快照、上次完成意见和新增轨迹。
轨迹包括模型可见文本（不含 reasoning）、工具调用/结果/错误、已投递消息；
工具内 patch/差异/handoff 可作为证据，不读取活跃工作区。轨迹窗口最多 120 个事件、
payload 共 20000 字符、每事件最多 1200 字符；范围和批准文档单独有界，先脱敏再裁剪，
标记遗漏事件和截断，不能把遗漏视为无进展。没有新的证据则不调模型。

审查者仅有 submit_trajectory_review 工具（verdict/evidence/recommendation 三个必填字符串，
严格闭合对象，不使用 uniqueItems）。结论限定 continue/refocus/suggest_suspend/insufficient_evidence，
本地只校验输出形状并保存，不调用控制、规划、Workspace 或 Gate 能力。
宿主记录 started/completed/failed/cancelled、独立 Session、覆盖序号与模型；
在开始前推进窗口，失败/中断不自动重放同一窗口。重启 watcher 的时间间隔从重新接管时起计，
已有派发窗口不重复消费；不承诺跨进程故障恢复已经实测。

公开入口：execution.json 的 trajectory_review，API 宿主的 --trajectory-review 与两个阈值参数；
get-run-trajectory-reviews 和 GET /runs/{run_id}/trajectory-reviews 查询最新审查状态。
查询不受通用 get-trace 的总事件截断影响，保留 Worker 当前状态及审查独立 reported_usage，
不把建议当成当前 Worker 已执行的动作。HTTP Schema、执行配置 Schema 和生成 Client 同步。

正常入口试用（非唯一产品 E2E）：

- 仓库外 ehai-trajectory-trial-20260914，使用 CLI 创建/规划/批准/execute-plan；
  显式 single 演示 Planner 不花模型 token，不是 Pi Planner 生成图的验收。
  初次测试配置把可执行文件写为绝对路径，既有 command 工具要求裸命令名，启动前失败；
  Run aa6ab79c-0273-4d19-8739-a05a124f4e35 为 paused，无模型调用。只修正临时配置为 pwsh，
  未放宽产品命令安全规则，旧记录和授权保持不变。
- 相同任务通过新的正常批准进入 Run 169e25cf-e8e4-42f8-b27b-ecf102118fc9，
  Worker Session 6c679583-fc91-4a7e-8264-0d192e305e80。配置 600 秒/2 步仅加速本次验证。
  Worker 两次读取参数后执行唯一批准的 25 秒等待/输出命令，再提交结果，共 4 个有效执行步。
- 第 2 步触发 review c0d654b3-f2e5-49fa-81ff-6a8cf9c6c58c，独立 Session
  a91b6dc9-e524-4e27-9b67-f9e487f05122。读取固定序号 1–11 窗口，Luna 调用提交意见工具，
  verdict=continue，证据引用读取记录；意见完成时 Worker 仍 running、已推进到后续轨迹。
  原命令仅运行一次，Worker 没有接收审查消息，最终正常 completed。没有审查者工作区操作。
- 正常 CLI 重读同一意见；生产 API 装配下调用新 GET 路由返回 HTTP 200、同一 completed 意见。
  Worker reported totalTokens=10676，审查 reported totalTokens=1578，合计 12254；均为 Luna。
  原仓库没有代码差异，核对两个 Pi 进程已退出，没有补审已完成任务。
- Ruff/格式/mypy（106 源文件）、Schema 生成和 Client generate/typecheck/build 通过。
  没有新增长期测试或额外 E2E 套件；本次工具试用不证明所有漂移判断都正确。

剩余验证：真实 600 秒触发、无新轨迹跳过的长时间场景、失败/取消/崩溃恢复、跨 Worker 压力，
以及 refocus/suggest_suspend 的实际判断质量。尚未把 Pi 用量接入全局预算自动停止。
