...
用户随后明确批准将当前工作树已有执行能力、自举记录和 Pi 后端迁移按逻辑提交并合入 main。
下文 M1 的独立迁移边界保留为历史，不能据其“暂缓”标签推断当前主线仍只有 schema 11。
本次保留 main 的共享 run_results 投影、HTTP /result 和生成 Client，同时保留 Pi 分支的
阶段/过程/成果接手字段。统一 trace 提取使用当前 RunView 与 AgentTraceReader，CLI 不再重复
实现结果投影，普通及 P2 API 均显式传入 artifact_root。

合流验证：Ruff、格式、104 源文件 mypy、Client generate/typecheck/build 通过。
复用 tests/test_self_hosting.py 的 verify-result 子命令，对真实完成 Run
5bcdbe79-0c1d-49dc-85ae-0e7f6b1c4f24 的只读 backup 副本验证 CLI、HTTP、生成 Client 和未知 Run
错误，结果通过，证据位于本机临时目录 ehai-result-gate-ooty3j87。原数据库未修改，未调用模型。
这只证明结果查询合流行为，不证明 Pi 工具执行、上下文压缩或完整两代自举 E2E 已通过。
